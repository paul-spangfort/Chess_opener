  ...
  // we're only interested in position information
    ...
    // loop through each character in the FEN section
      ...
      // piece
          ...
          const square = COLUMNS[colIndex] + currentRow;
...
//------------------------------------------------------------------------------
    ...
    let MINIMUM_JQUERY_VERSION = '1.7.0',
...
// http://stackoverflow.com/questions/105034/how-to-create-a-guid-uuid-in-javascript
    function createId() {
      return 'xxxx-xxxx-xxxx-xxxx-xxxx-xxxx-xxxx-xxxx'.replace(/x/g, (c) => {
...
// returns true if version is >= minimum
    function compareSemVer(version, minimum) {
      ...
      const versionNum = (version.major * 10000 * 10000) +
    (version.minor * 10000) + version.patch;
      const minimumNum = (minimum.major * 10000 * 10000) +
...
// check dependencies
  ...
  // if containerId is a string, it must be the ID of a DOM node
    ...
    // make sure the container element exists in the DOM
        const el = document.getElementById(containerElOrId);
        if (!el) {
...
// our square size
    function calculateSquareSize() {
      const containerWidth = parseInt(containerEl.css('width'), 10);
...
//------------------------------------------------------------------------------
  ...
  // get information about the source and destination squares
      const srcSquareEl = $(`#${SQUARE_ELS_IDS[src]}`);
      const srcSquarePosition = srcSquareEl.offset();
      const destSquareEl = $(`#${SQUARE_ELS_IDS[dest]}`);
      const destSquarePosition = destSquareEl.offset();

  // create the animated piece and absolutely position it
  // over the source square
      const animatedPieceId = createId();
      $('body').append(buildPiece(piece, true, animatedPieceId));
      const animatedPieceEl = $(`#${animatedPieceId}`);
      animatedPieceEl.css({
        display: '',
        position: 'absolute',
  ...
  // animate the piece to the destination square
    ...
    function animateSparePieceToSquare(piece, dest, completeFn) {
      const srcOffset = $(`#${SPARE_PIECE_ELS_IDS[piece]}`).offset();
      const destSquareEl = $(`#${SQUARE_ELS_IDS[dest]}`);
      const destOffset = destSquareEl.offset();

  // create the animate piece
      const pieceId = createId();
      $('body').append(buildPiece(piece, true, pieceId));
      const animatedPieceEl = $(`#${pieceId}`);
      animatedPieceEl.css({
        display: '',
        position: 'absolute',
        left: srcOffset.left,
...
// returns the distance between two squares
    function squareDistance(s1, s2) {
      s1 = s1.split('');
      const s1x = COLUMNS.indexOf(s1[0]) + 1;
      const s1y = parseInt(s1[1], 10);

      s2 = s2.split('');
      const s2x = COLUMNS.indexOf(s2[0]) + 1;
      const s2y = parseInt(s2[1], 10);

      const xDelta = Math.abs(s1x - s2x);
      const yDelta = Math.abs(s1y - s2y);
...
//------------------------------------------------------------------------------
  ...
  // add the pieces
      for (const i in CURRENT_POSITION) {
...
// with the moves executed
    ...
    function setCurrentPosition(position) {
      const oldPos = deepCopy(CURRENT_POSITION);
  ...
  // update state
    ...
    function isXYOnSquare(x, y) {
      for (const i in SQUARE_ELS_OFFSETS) {
        if (SQUARE_ELS_OFFSETS.hasOwnProperty(i) !== true) continue;

        const s = SQUARE_ELS_OFFSETS[i];
...
// records the XY coords of every square into memory
  ...
  // remove the source piece
      const newPosition = deepCopy(CURRENT_POSITION);
  ...
  // get target square information
      const targetSquarePosition = $(`#${SQUARE_ELS_IDS[square]}`).offset();
  ...
  // run their onDrop function, which can potentially change the drop action
    ...
    // source piece was on the board and position is on the board
      ...
      // move the piece
        ...
        const oldPosition = deepCopy(CURRENT_POSITION);

        const result = cfg.onDrop(DRAGGED_PIECE_SOURCE, location, DRAGGED_PIECE,
...
// move pieces
  ...
  // calculate position from moves
      const newPos = calculatePositionFromMoves(CURRENT_POSITION, moves);
...
// reference: http://www.quirksmode.org/js/detect.html
  ...
  // get the location
      const location = isXYOnSquare(e.pageX, e.pageY);
  ...
  // get the location
      const location = isXYOnSquare(e.originalEvent.changedTouches[0].pageX,
    e.originalEvent.changedTouches[0].pageY);
